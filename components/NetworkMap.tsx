...
const NetworkMap: React.FC = () => {
  ...
  // Mojokerto (HQ) di tengah agak bawah
  // Gedeg di utara sungai
  // Gempol Kerep di barat laut
  // Ngudi Kidul di selatan
  const locations = [
  ...
  return (
    <div className="relative w-full aspect-[16/9] md:aspect-[2/1] bg-neutral-950 rounded-xl overflow-hidden border border-neutral-800 group">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 600 350">
        ...
        <path d="M-50,200 C150,250 300,100 450,200 S700,180 800,250"
              fill="none" stroke="#1a1a1a" strokeWidth="30" strokeLinecap="round" />
        ...
        {/* Nodes */}
        {locations.map((loc, i) => (
          <g key={loc.id} className="cursor-pointer hover:opacity-100">
             ...
             {/* Core Node */}
             <circle cx={loc.x} cy={loc.y} r={loc.type === 'hq' ? 5 : 3} fill={loc.type === 'hq' ? '#fff' : '#888'} filter="url(#glow)" />

             {/* Label Lines */}
             <line x1={loc.x} y1={loc.y} x2={loc.x + 15} y2={loc.y - 15} stroke="#333" strokeWidth="1" />
             <line x1={loc.x + 15} y1={loc.y - 15} x2={loc.x + 100} y2={loc.y - 15} stroke="#333" strokeWidth="1" />
             
             {/* Text Label (SVG Text for precise positioning) */}
             <foreignObject x={loc.x + 20} y={loc.y - 35} width="120" height="40">
               <div className="flex flex-col items-start">
                 <span className={`text-[10px] font-bold ${loc.type === 'hq' ? 'text-white' : 'text-neutral-400'} bg-neutral-950/50 px-1`}>{loc.name}</span>
                 <span className="text-[8px] font-mono text-neutral-600 bg-neutral-950/50 px-1 mt-0.5 tracking-tighter">STATUS: {loc.status}</span>
               </div>
             </foreignObject>
          </g>
        ))}
      ...
      <div className="absolute bottom-4 left-4 z-10 flex flex-col gap-1">
         <div className="flex items-center gap-2 text-[10px] text-neutral-500">
            <span className="w-2 h-2 rounded-full bg-white"></span> HUB (HQ)
         </div>
         <div className="flex items-center gap-2 text-[10px] text-neutral-500">
